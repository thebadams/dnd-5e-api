...
describe('Spell Schema', () => {
	...
	describe('Spell Model Validation', () => {
		...
		let badSpellOptions: mongoose.Error.ValidationError;
		beforeAll(async () => {
			const components: IComponents = {
				v: true,
				s: true,
			...
			badSpell = await Spell.create({}).catch((e) => e);
			badSpellOptions = await Spell.create(badSpellInfo).catch((e) => e);
		...
		describe('Level Validation', () => {
			...
			test('If A Spell Level, Not Of Numbers 0-9 Are Passed In, Throw A Validation Error', () => {
				expect(badSpellOptions).toBeInstanceOf(mongoose.Error.ValidationError);
			});
			test('If A Spell Level Error Is Thrown, Message Should Be "10 Is Not A Valid Spell Level"', () => {
				...
				expect(badSpellOptions).toHaveProperty('errors', expect.objectContaining(expected));
			});
		});
